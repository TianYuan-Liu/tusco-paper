...
data_root <- file.path(repo_root, "data", "raw")
...
# Default (legacy) TUSCO file; callers may override per tissue
tusco_file <- if (!local_only) file.path(repo_root, "data", "processed", "tusco", "mmu", "tusco_mouse.tsv") else NA_character_

# Resolve tissue-specific TUSCO list (mouse brain/kidney)
get_tusco_mouse_tsv <- function(tissue = c("Brain","Kidney")) {
  tissue <- match.arg(tissue)
  fname <- if (tissue == "Brain") "tusco_mouse_brain.tsv" else "tusco_mouse_kidney.tsv"
  candidates <- c(
    file.path(repo_root, "data", "processed", "tusco", "mmu", fname),
    file.path(repo_root, "data", "raw", "tusco", "mmu", fname)
  )
  hits <- candidates[file.exists(candidates)]
  if (length(hits) > 0) return(hits[1])
  # fallback to generic file to avoid hard failure
  file.path(repo_root, "data", "processed", "tusco", "mmu", "tusco_mouse.tsv")
}
...
# Helper: return the first existing path from a list
...
# ------------------------------------------------------------
compute_metrics_from_classification <- function(classification_data, tusco_tsv = tusco_file) {
  ...
  if (is.null(tusco_tsv) || is.na(tusco_tsv) || !file.exists(tusco_tsv)) {
    stop("Required TUSCO reference not found: ", tusco_tsv)
  }
  tusco_lines <- readLines(tusco_tsv)
...
collect_points_for_metrics <- function(metrics_key) {
  ...
  for (sd in single_sample_dirs) {
    ...
    tusco_tsv <- get_tusco_mouse_tsv(tissue)
    m <- compute_metrics_from_classification(classification, tusco_tsv)
    mv <- m[[metrics_key]]
    df <- tibble(
      Tissue = tissue,
      Samples = 1L,
...
if (!local_only) {
  ...
  # LOCAL_ONLY mode: read precomputed data from plot_dir
...
if (!local_only) {
  ...
  tusco_universal_tsv <- first_existing(c(
    file.path(repo_root, "data", "processed", "tusco", "mmu", "tusco_mouse.tsv"),
    file.path(data_root, "tusco", "tusco_mouse.tsv")
  ))
  tusco_brain_tsv <- first_existing(c(
    file.path(repo_root, "data", "processed", "tusco", "mmu", "tusco_mouse_brain.tsv"),
    file.path(data_root, "tusco", "tusco_mouse_brain.tsv")
  ))
  tusco_kidney_tsv <- first_existing(c(
    file.path(repo_root, "data", "processed", "tusco", "mmu", "tusco_mouse_kidney.tsv"),
    file.path(data_root, "tusco", "tusco_mouse_kidney.tsv")
  ))

  message("[fig-5] TUSCO universal TSV: ", tusco_universal_tsv)
  message("[fig-5] TUSCO brain TSV: ", tusco_brain_tsv)
  message("[fig-5] TUSCO kidney TSV: ", tusco_kidney_tsv)
