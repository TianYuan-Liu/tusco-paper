...
utils::globalVariables(c(
  ...
  "Samples", "gene", "chrom", "type", "n", "Mean", "SD", "Type",
  "Sample", "Combo", "Sn", "nrPre", "1/red", "1-FDR", "PDR", "rPre",
  "Max", "Min", ".pt", "N", "SE", "CI_t", "CI_lower", "CI_upper"
))
...
out_pdf <- file.path(plot_dir, "final-fig-5b-5c.pdf")
...
# 1) Single-sample points from single_sample (B31–B35, K31–K35)
...
for (sd in single_sample_dirs) {
  ...
  df <- tibble(
    ...
    Samples = 1L,
    Combo = sample_name,
    Metric = names(m$metrics_tusco),
    Value = as.numeric(m$metrics_tusco)
  )
  single_points[[length(single_points) + 1]] <- df
}
single_points_df <- dplyr::bind_rows(single_points)

# 2) Multi-sample combinations from intersection_all_comb
...
# Keep only pure Brain or pure Kidney combinations (no mixed B and K together)
...
all_points_df <- bind_rows(single_points_df, combo_points_df)
...
# Aggregate: mean and SD per tissue x samples x metric
summary_df <- all_points_df %>%
...
create_metric_barplot_all <- function(metric_name, points_df, summary_df, color_value = "#2ca02c") {
  ...
  improvement_data <- metric_summary %>%
    filter(Samples %in% c(1, 2), Tissue %in% c("Brain", "Kidney")) %>%
    arrange(Tissue, Samples) %>%
    group_by(Tissue) %>%
    filter(n() == 2) %>%
    summarise(
      improvement = Mean[Samples == 2] - Mean[Samples == 1],
      sample1_value = Mean[Samples == 1],
      sample2_value = Mean[Samples == 2],
      .groups = "drop"
    ) %>%
    mutate(
  ...
  ggplot() +
    ...
    # Improvement label between 1 and 2 samples per facet
    geom_text(
...
# ------------------------------------------------------------
# Export detailed data points used for the panels as TSVs
# ------------------------------------------------------------
points_5b_tsv <- file.path(plot_dir, "final-fig-5b_points.tsv")
points_5c_tsv <- file.path(plot_dir, "final-fig-5c_points.tsv")
bars_5b_tsv <- file.path(plot_dir, "final-fig-5b_bars.tsv")

all_points_out <- all_points_df %>%
  arrange(Tissue, Samples, Combo, Metric)
readr::write_tsv(all_points_out, points_5b_tsv)

bar_means_out <- summary_df %>%
  arrange(Tissue, Samples, Metric) %>%
  select(Tissue, Samples, Metric, N, Mean, SD, SE, CI_lower, CI_upper)
readr::write_tsv(bar_means_out, bars_5b_tsv)

metrics_points_out <- metrics_df %>%
  arrange(Tissue, Type, Sample)
readr::write_tsv(metrics_points_out, points_5c_tsv)

message("Wrote 5b data points to ", points_5b_tsv)
message("Wrote 5b bar values to ", bars_5b_tsv)
message("Wrote 5c radar data points to ", points_5c_tsv)
